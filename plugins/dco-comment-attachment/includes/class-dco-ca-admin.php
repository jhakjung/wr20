...
class DCO_CA_Admin extends DCO_CA_Base {
	...
	public function add_comment_action_links( $actions, $comment ) {
		if ( $this->has_attachment() ) {
			...
			$title                              = esc_html__( '첨부파일 삭제', 'dco-comment-attachment' );
	...
	public function add_comments_bulk_actions( $actions ) {
		$actions['deleteattachment'] = __( '첨부파일 삭제', 'dco-comment-attachment' );
	...
	public function show_bulk_action_message( $translation, $single, $plural, $number, $domain ) {
		...
		if ( '%s comment approved.' === $single && '%s comments approved.' === $plural && 'default' === $domain ) {
			...
			$translation = sprintf( _n( '%s개의 댓글에서 첨부파일 삭제됨.', '%s개의 댓글에서 첨부파일 삭제됨.', $number, 'dco-comment-attachment' ), $number );
		}
	...
	public function add_attachment_metabox() {
		add_meta_box( 'dco-comment-attachment', esc_html__( '첨부파일', 'dco-comment-attachment' ), array( $this, 'render_attachment_metabox' ), 'comment', 'normal' );
	}
	...
	public function render_attachment_metabox() {
			...
			foreach ( (array) $attachment_id as $attach_id ) :
				...
				<div class="dco-attachment-wrap">
					...
					<div class="dco-attachment-actions">
						<a href="#" class="button dco-set-attachment"><?php esc_html_e( '파일 교체', 'dco-comment-attachment' ); ?></a>
						<a href="#" class="dco-remove-attachment"><?php esc_html_e( '파일 제거', 'dco-comment-attachment' ); ?></a>
					</div>
		...
		<div class="dco-attachment-wrap">
			<div class="dco-attachment-notice dco-hidden"><?php echo wp_kses_data( __( 'Update the comment to see a preview of <a href="#" target="_blank">the selected attachment</a>.', 'dco-comment-attachment' ) ); ?></div>
			<div class="dco-attachment-actions">
				<a href="#" class="button dco-set-attachment"><?php echo esc_html_e( '파일 추가', 'dco-comment-attachment' ); ?></a>
				<a href="#" class="dco-remove-attachment dco-hidden"><?php esc_html_e( '파일 제거', 'dco-comment-attachment' ); ?></a>
			</div>
