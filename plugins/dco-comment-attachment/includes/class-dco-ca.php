		...
		switch ( $type ) {
			case 'label':
				...
				<label class="comment-form-attachment__label" for="attachment">
					<?php
					$label = $this->get_option( 'enable_multiple_upload' ) ? __( '첨부할 ', 'dco-comment-attachment' ) : __( 'Attachment', 'dco-comment-attachment' );
					echo esc_html( $label );
					if ( $required ) {
	...
	public function check_attachment( $prepared_comment ) {
		...
		foreach ( $names as $name ) {
			...
			if ( ! $filetype['ext'] ) {
				return new WP_Error(
					'dco-comment-attachment',
					__( "해당 형태의 파일타입은 업로드 불가합니다. 관리자에게 문의하세요.", 'dco-comment-attachment' ),
					array( 'status' => 400 )
				);
			}
		}
