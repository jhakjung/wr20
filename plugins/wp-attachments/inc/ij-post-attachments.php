...
class IJ_Post_Attachments
{
	...
	public function printMetaBox($post)
	{
		$attachments = new WP_Query(array(
			...
			'order'         => 'ASC',
            'posts_per_page' => -1,
			'exclude' => get_option('thread_comments') ? get_option('thread_comments') : ''
		));
...
<div id="ij-post-attachments" class="ij-post-attachment-list">
  <ul class="not-show">
    ...
    <li class="ij-post-attachment" data-mimetype="<?php echo $atchment->post_mime_type; ?>" data-alt="<?php echo esc_attr(get_post_meta(611, '_wp_attachment_image_alt', true)); ?>" data-attachmentid="<?php echo $atchment->ID; ?>" data-url="<?php echo wp_get_attachment_url($atchment->ID); ?>" data-title="<?php echo esc_attr($atchment->post_title); ?>">
    ...
    <td width="30%" style="text-align: right;vertical-align: middle;">
      <span>
        ...
        <a class="button button-small " href="<?php echo esc_url( admin_url('tools.php?page=unattach&noheader=true&id=' . $atchment->ID) );?>" onclick="if (! confirm('<?php _e('Are you sure you want to do this?');?>')) { return false; }" title="<?php echo _e('Unattach','wp-attachments'); ?>">
