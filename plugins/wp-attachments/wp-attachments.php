...
function wpatt_content_filter($content, $post = null) {
    ...
    $checkrestrict = false;
    if (get_option('wpatt_option_restrictload') && !is_single() && !is_page()) {
        $checkrestrict = true;
    }

    if ($post->ID == '0' || $post->ID == NULL || get_post_meta($post->ID, 'wpa_off', true) || post_password_required() || $checkrestrict) {
        return $content;
    }
    ...
    if ($attachments) {
        ...
        foreach ($attachments as $attachment) {
            // Check if attachment belongs to a comment
            $attachment_comment = get_post($attachment->post_parent);
            if ($attachment_comment && $attachment_comment->post_type === 'comment') {
                continue; // Skip attachment if it belongs to a comment
            }
            ...
            $attachment_icon = wp_get_attachment_image($attachment->ID, array(24, 24));

            $attachment_title = apply_filters('the_title', $attachment->post_title);

            if (get_option('wpatt_option_showsize') == 1) {
                $attachment_size = wpatt_get_size($attachment->ID);
            } else {
                $attachment_size = '';
            }
